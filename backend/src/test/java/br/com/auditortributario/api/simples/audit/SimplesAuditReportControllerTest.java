package br.com.auditortributario.api.simples.audit;

import org.junit.jupiter.api.Test;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.webmvc.test.autoconfigure.AutoConfigureMockMvc;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc;

import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

@SpringBootTest
@AutoConfigureMockMvc
class SimplesAuditReportControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void shouldGenerateDivergentAuditReportThroughHttp()
            throws Exception {

        String requestBody = """
                {
                  "calculation": {
                    "openingDate": "2024-01-10",
                    "assessmentPeriod": "2026-01",
                    "fatorRPayrollBase": 150000.00,
                    "fatorRRevenueBase": 500000.00,
                    "taxableRevenue": 10000.00,
                    "priorMonthlyRevenues": [
                      {
                        "period": "2025-01",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-02",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-03",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-04",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-05",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-06",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-07",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-08",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-09",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-10",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-11",
                        "amount": 41666.66
                      },
                      {
                        "period": "2025-12",
                        "amount": 41666.74
                      }
                    ]
                  },
                  "guideAmount": 1752.00,
                  "reportedFatorR": 0.25,
                  "reportedAnnex": "ANEXO_V",
                  "reportedBracketNumber": 3,
                  "reportedEffectiveRate": 0.1752
                }
                """;

        mockMvc.perform(
                post(
                        "/api/v1/simples/audit/report")
                        .contentType(
                                MediaType.APPLICATION_JSON)
                        .accept(
                                MediaType.APPLICATION_JSON)
                        .content(
                                requestBody))
                .andExpect(
                        status().isOk())

                .andExpect(
                        jsonPath("$.assessmentPeriod")
                                .value("2026-01"))

                .andExpect(
                        jsonPath("$.status")
                                .value("DIVERGENT"))

                .andExpect(
                        jsonPath("$.severity")
                                .value("HIGH"))

                .andExpect(
                        jsonPath("$.principalCause")
                                .value(
                                        "FACTOR_R_OR_ANNEX"))

                .andExpect(
                        jsonPath("$.title")
                                .value(
                                        "Relatório de Auditoria Tributária"))

                .andExpect(
                        jsonPath("$.executiveSummary")
                                .isString())

                .andExpect(
                        jsonPath("$.sections.length()")
                                .value(6))

                .andExpect(
                        jsonPath("$.sections[0].code")
                                .value("SUMMARY"))

                .andExpect(
                        jsonPath(
                                "$.sections[1].code")
                                .value(
                                        "TAX_FRAMEWORK"))

                .andExpect(
                        jsonPath(
                                "$.sections[2].code")
                                .value(
                                        "AMOUNT_COMPARISON"))

                .andExpect(
                        jsonPath(
                                "$.sections[3].code")
                                .value(
                                        "FINDINGS"))

                .andExpect(
                        jsonPath(
                                "$.sections[4].code")
                                .value(
                                        "RECOMMENDATIONS"))

                .andExpect(
                        jsonPath(
                                "$.sections[5].code")
                                .value(
                                        "TRACEABILITY"))

                .andExpect(
                        jsonPath("$.references")
                                .isArray())

                .andExpect(
                        jsonPath("$.markdown")
                                .isString());
    }

    @Test
    void shouldGenerateCompatibleAuditReportThroughHttp()
            throws Exception {

        String requestBody = """
                {
                  "calculation": {
                    "openingDate": "2026-02-10",
                    "assessmentPeriod": "2026-02",
                    "fatorRPayrollBase": 6000.00,
                    "fatorRRevenueBase": 20000.00,
                    "taxableRevenue": 20000.00,
                    "priorMonthlyRevenues": []
                  },
                  "guideAmount": 1460.00,
                  "reportedFatorR": 0.30,
                  "reportedAnnex": "ANEXO_III",
                  "reportedBracketNumber": 2,
                  "reportedEffectiveRate": 0.073
                }
                """;

        mockMvc.perform(
                post(
                        "/api/v1/simples/audit/report")
                        .contentType(
                                MediaType.APPLICATION_JSON)
                        .accept(
                                MediaType.APPLICATION_JSON)
                        .content(
                                requestBody))
                .andExpect(
                        status().isOk())

                .andExpect(
                        jsonPath("$.status")
                                .value("COMPATIBLE"))

                .andExpect(
                        jsonPath("$.severity")
                                .value("NONE"))

                .andExpect(
                        jsonPath("$.principalCause")
                                .value("NONE"))

                .andExpect(
                        jsonPath("$.markdown")
                                .value(
                                        org.hamcrest.Matchers
                                                .containsString(
                                                        "# Relatório de Auditoria Tributária")))

                .andExpect(
                        jsonPath("$.markdown")
                                .value(
                                        org.hamcrest.Matchers
                                                .containsString(
                                                        "02/2026")));
    }
}