...
public record SimplesCompetenceRevenueItemResult(
        ...
        Optional<SimplesGoodsRevenueTaxResult> taxResult,
        String explanation) {
            ...
            if (taxResult.isPresent()) {
                throw new IllegalArgumentException(
                        "Uma rota ainda não suportada "
                                + "não pode possuir cálculo tributário final.");
            }

            return;
        }

        SimplesGoodsRevenueTaxResult result = taxResult.orElseThrow(
        ...
        if (status == SimplesCompetenceRevenueItemStatus.COMPLETED_WITH_EXTERNAL_OBLIGATION
            ...
            throw new IllegalArgumentException(
                    "O status informa obrigação externa, "
                            + "mas o resultado tributário não possui essa obrigação.");
        }
